...
def process_hg(h, M):
    ...
    if w.shape[0] == 0:
        raise ValueError("La matrice di incidenza `w` ha 0 righe: non posso calcolare i pesi.")

    weights = np.full((w.shape[0], ), 1 / w.shape[0])
    return w, weights, v    
"""
def incidence(g, M, N):
    A = np.zeros((M, N))
...
def incidence(g, M, N):
    A = np.zeros((M, N))
    # g.edges è un dict: chiave=edge_id, valore=set di nodi
    for j, (edge_id, nodes_set) in enumerate(g.edges.items()):
        node_indices = np.array([int(node) - 1 for node in nodes_set])
        A[node_indices, j] = 1
    return A    
"""
def incidence(g, M, N):
    ...
    # Crea una mappatura nodo → indice per garantire coerenza
    nodes = sorted(g.nodes)
    node_to_idx = {str(node): i for i, node in enumerate(nodes)}

    for j, (edge_id, nodes_set) in enumerate(g.incidence_dict.items()):
        for node in nodes_set:
            node_str = str(node)
            if node_str in node_to_idx:
                A[node_to_idx[node_str], j] = 1
            else:
                raise ValueError(f"Nodo {node_str} non trovato in node_to_idx")
